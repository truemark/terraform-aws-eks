...
#         },
#         { for k, v in {
#             iam_role_arn         = try(module.keda.iam_role_arn, null)
#             namespace            = local.keda_namespace
#             service_account_name = local.keda_service_account
#             } : "keda_${k}" => v if var.enable_keda
#         },
#         { for k, v in {
#             iam_role_arn         = try(module.aws_load_balancer_controller.iam_role_arn, null)
...
#         },
#         { for k, v in {
#             iam_role_arn         = try(module.karpenter[0].iam_role_arn, null)
#             node_iam_role_arn    = try(module.karpenter[0].node_iam_role_name, null)
#             interruption_queue   = try(module.karpenter[0].queue_name, null)
#             namespace            = local.karpenter_namespace
#             service_account_name = local.karpenter_service_account
#             } : "karpenter_${k}" => v if var.enable_karpenter
#         },
#         { for k, v in {
#             iam_role_arn            = try(module.velero.iam_role_arn, null)
...
#         },
#         { for k, v in {
#             thanos_iam_role_arn     = var.enable_observability && var.observability.thanos.enabled ? module.thanos.iam_role_arn: ""
#             thanos_s3_bucket_name   = var.enable_observability && var.observability.thanos.enabled ? module.thanos_s3_bucket.s3_bucket_id: ""
#             prometheus_iam_role_arn = var.enable_observability && var.observability.kube_prometheus_stack.enabled ? module.prometheus_iam_role[0].iam_role_arn : ""
